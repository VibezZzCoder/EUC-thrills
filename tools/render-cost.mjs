...
const { PART_COSTS, RENDER_BUDGET, propPartCounts } = await import(join(src, 'data/renderCost.ts'));
...
out('```');
out();
out('"Everything else" is the rider rig, the ghost, the checkpoint gates, both');
out('particle fields, and three\'s own background pass, measured in the most');
out('expensive state a player can reach — a timed run with the gates up and a');
out('saved ghost on course. Free ride costs materially less: both start hidden,');
out('and an invisible subtree draws nothing.');
out();
