...
What you can play today is the first complete slice of it: **one hand-built
loop, two modes.** It runs start to finish, it is not a demo that stops
halfway, and the riding is the part that is meant to be right — but it is the
foundation of the game rather than the whole of it.

Worth knowing before you start:

- The world is **one route**. There is no second map yet, and no way to
  generate your own.
...
- **Your saved times may not survive every update.** Best laps and ghosts are
  stored against the course as it is now; if the course or the save format
  changes, old records are left behind rather than silently converted into
...
*Free ride and the time trial share one world. The city, the park, the river
crossing, the trail, and everything on them are generated by the game at
startup, so the repository is small. It holds the page itself (`index.html`
with its built script and stylesheet under `assets/`), four sound recordings —
the only media the game loads — the gameplay screenshot above under `media/`,
the licence texts (`LICENSE`, `NOTICE.md`), a `.nojekyll` marker, and
`SHA256SUMS.txt`, a manifest listing the checksum of every published file
except itself.*

## Riding

You are **Cool Rider**, on a fictional suspension wheel. The whole ride is one
...
is genuinely faster and genuinely riskier.
...
| Camera view | Y |
| Pause | Start |
| In menus | Left stick or D-pad to move, A to confirm, B to go back |
...
**Start ride** is free ride: no clock, no objective, nothing to fail. Ride the
loop, or ignore it entirely and go and look at the river.

**Time trial** runs the same loop against a start line and five checkpoints.
The HUD points at the next one with a bearing and a distance; each crossing
shows the split and, once you have a time to beat, how far ahead or behind it
you are. The results screen breaks the run down leg by leg.

Every checkpoint sits on ground both routes share, so the alley shows up where
it actually happens — as a faster leg — and a time set the safe way is still
comparable with one set through the alley.
...
- **A route that generates itself.** The world is already drawn by code, but
  its layout is hand-authored and identical every ride. The next milestone
  makes the route itself procedural: a library of hand-tuned set pieces —
  plaza, kerb run, S-bend boulevard, alley shortcut, park gate, grass cut,
  gravel spur, berm, root garden, kicker, climb — stitched into a fresh loop
  from a seed, and validated so every jump is landable at the speed the
  approach actually produces and every shortcut reconnects. Authored pieces
  composed by a generator, rather than noise. The seed becomes part of the
  interface, because a ghost only means anything against the same one.
- **A proper look pass.** The world is drawn entirely by the game rather than
  loaded from models, which leaves a lot of room to make it better looking —
  and once routes are generated there is far more of it on screen, so the pass
  is as much about keeping the frame rate as it is about the picture.
- **Wobble, redesigned.** Speed wobble is the missing third EUC risk. It needs
  to be recoverable and readable without being the thing that ruins your ride,
  which the first attempt was.
- **A measured performance pass.** The target is a steady 60 fps at 1080p on
  integrated graphics. The tooling to measure that honestly exists; the
  measurement itself is still owed.
